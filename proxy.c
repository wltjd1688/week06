...
void doit(int fd) {
  ...
  rio_t request_rio, response_rio;                                                    // 송원이랑 소정이말로는 rio를 두 개 쓸 필요없이 하나 끝나면 닫고 다시 열고 하면 된기에 하나만 써도 된다고한다.
                                                                                      // 리뷰쓴다면 이거 언급해도될듯
  ...
  sscanf(buf, "%s %s %s", method, uri, version);                                      // buf에는 클라이언트에서 보낸 요청이 들어오게된다. 그걸 method, uri, version을 기준으로 
                                                                                      // 스페이스간격만큼 떨어져있기에 가각의 변수에 넣을 수 있게된다.
  ...
  int clientfd = Open_clientfd(hostname, host_port);                                  // 서버로 보내야 하는 fd를 연다. clientfd로 지은 이유는 서버입장에서 proxy가 클라이기 때문이라고한다.

  char request_buf[MAXBUF], response_buf[MAXBUF];
  int content_length;

  //-----------------------------------헤더---------------------
  sprintf(request_buf, "Host: %s\r\n", host_uri);
